class RecipeCategoriesController < ApplicationController
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to recipe_categories_path, status: :see_other, notice: "Recipe category was successfully destroyed." }
  ...
  private
    ...
    def set_recipe_category
      @recipe_category = RecipeCategory.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def recipe_category_params
      params.expect(recipe_category: [ :category_name ])
    end
end
